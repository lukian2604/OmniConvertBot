LANGUAGES = {
    "ru": "🇷🇺 Русский",
    "en": "🇬🇧 English",
    "it": "🇮🇹 Italiano",
    "ro": "🇷🇴 Română",
    "pt": "🇵🇹 Português",
    "ar": "🇸🇦 العربية",
    "ja": "🇯🇵 日本語",
    "zh": "🇨🇳 中文",
}

MESSAGES = {
    "ru": {
        "welcome": (
            "Добро пожаловать в Конвертер файлов - универсальный бот для мгновенного "
            "преобразования файлов всех популярных форматов!\n\n"
            "📌 Поддерживаемые типы файлов:\n"
            "📷 Изображения: png, jpg, webp, heic, svg и др.\n"
            "🎬 Видео и гифки: mp4, avi, mov, webm → даже в mp3 или gif!\n"
            "🎵 Аудио: mp3, flac, ogg, wav, m4a\n"
            "📄 Документы: docx, xlsx, pptx, txt, pdf, html и др.\n\n"
            "Просто отправьте файл — бот сам предложит доступные форматы для конвертации.\n"
            "⚡ Быстро. Удобно. Без регистрации."
        ),
        "converting": "🔄 Конвертирую файл, подождите немного...",
        "format_error": "⚠️ К сожалению, этот формат файлов пока не поддерживается или файл поврежден.",
        "text_error": "📥 Пожалуйста, отправьте мне файл, который вы хотите конвертировать.",
        "done": "✅ Готово! Ваш файл успешно конвертирован.",
        "pick_format": "➡️ Выберите формат конвертации:",
        "file_too_large": "⚠️ Файл слишком большой. Максимальный размер для загрузки — 20 МБ.",
        "result_too_large": "⚠️ Конвертированный файл превышает 50 МБ и не может быть отправлен через Telegram.",
    },
    "en": {
        "welcome": (
            "Welcome to File Converter - the universal bot for instant conversion of all "
            "popular file formats!\n\n"
            "📌 Supported file types:\n"
            "📷 Images: png, jpg, webp, heic, svg, etc.\n"
            "🎬 Video & GIFs: mp4, avi, mov, webm → even to mp3 or gif!\n"
            "🎵 Audio: mp3, flac, ogg, wav, m4a\n"
            "📄 Documents: docx, xlsx, pptx, txt, pdf, html, etc.\n\n"
            "Just send a file — the bot will automatically suggest the available formats.\n"
            "⚡ Fast. Convenient. No registration required."
        ),
        "converting": "🔄 Converting file, please wait a moment...",
        "format_error": "⚠️ Unfortunately, this file format is not supported yet or the file is corrupted.",
        "text_error": "📥 Please send me the file you want to convert.",
        "done": "✅ Done! Your file has been successfully converted.",
        "pick_format": "➡️ Choose output format:",
        "file_too_large": "⚠️ File is too large. The maximum upload size is 20 MB.",
        "result_too_large": "⚠️ The converted file exceeds 50 MB and cannot be sent via Telegram.",
    },
    "it": {
        "welcome": (
            "Benvenuto su Convertitore File - il bot universale per la conversione istantanea "
            "di tutti i formati di file più popolari!\n\n"
            "📌 Tipi di file supportati:\n"
            "📷 Immagini: png, jpg, webp, heic, svg, ecc.\n"
            "🎬 Video & GIF: mp4, avi, mov, webm → anche in mp3 o gif!\n"
            "🎵 Audio: mp3, flac, ogg, wav, m4a\n"
            "📄 Documenti: docx, xlsx, pptx, txt, pdf, html, ecc.\n\n"
            "Invia un file — il bot ti proporrà automaticamente i formati disponibili.\n"
            "⚡ Rapido. Comodo. Senza registrazione."
        ),
        "converting": "🔄 Conversione del file in corso, attendi un momento...",
        "format_error": "⚠️ Sfortunatamente, questo formato di file non è ancora supportato o il file è danneggiato.",
        "text_error": "📥 Per favore, inviami il file che desideri convertire.",
        "done": "✅ Fatto! Il tuo file è stato convertito con successo.",
        "pick_format": "➡️ Scegli il formato di output:",
        "file_too_large": "⚠️ File troppo grande. La dimensione massima per l'invio è 20 MB.",
        "result_too_large": "⚠️ Il file convertito supera i 50 MB e non può essere inviato tramite Telegram.",
    },
    "ro": {
        "welcome": (
            "Bun venit la Convertor Fișiere - botul universal pentru conversia instantanee "
            "a tuturor formatelor de fișiere populare!\n\n"
            "📌 Tipuri de fișiere acceptate:\n"
            "📷 Imagini: png, jpg, webp, heic, svg etc.\n"
            "🎬 Video și GIF-uri: mp4, avi, mov, webm → chiar și în mp3 sau gif!\n"
            "🎵 Audio: mp3, flac, ogg, wav, m4a\n"
            "📄 Documente: docx, xlsx, pptx, txt, pdf, html etc.\n\n"
            "Trebuie doar să trimiteți un fișier — botul va sugera automat formatele disponibile.\n"
            "⚡ Rapid. Convenabil. Fără înregistrare."
        ),
        "converting": "🔄 Se convertește fișierul, vă rugăm să așteptați un moment...",
        "format_error": "⚠️ Din păcate, acest format de fișier nu este încă acceptat sau fișierul este deteriorat.",
        "text_error": "📥 Vă rugăm să îmi trimiteți fișierul pe care doriți să îl convertiți.",
        "done": "✅ Gata! Fișierul dvs. a fost convertit cu succes.",
        "pick_format": "➡️ Alegeți formatul de ieșire:",
        "file_too_large": "⚠️ Fișierul este prea mare. Dimensiunea maximă pentru încărcare este de 20 MB.",
        "result_too_large": "⚠️ Fișierul convertit depășește 50 MB și nu poate fi trimis prin Telegram.",
    },
    "pt": {
        "welcome": (
            "Bem-vindo ao Conversor de Arquivos - o bot universal para conversão instantânea "
            "de todos os formatos de arquivos populares!\n\n"
            "📌 Tipos de arquivos suportados:\n"
            "📷 Imagens: png, jpg, webp, heic, svg, etc.\n"
            "🎬 Vídeos e GIFs: mp4, avi, mov, webm → até para mp3 ou gif!\n"
            "🎵 Áudio: mp3, flac, ogg, wav, m4a\n"
            "📄 Documentos: docx, xlsx, pptx, txt, pdf, html, etc.\n\n"
            "Basta enviar um arquivo — o bot sugerirá automaticamente os formatos disponíveis.\n"
            "⚡ Rápido. Conveniente. Sem registro."
        ),
        "converting": "🔄 Convertendo o arquivo, por favor aguarde um momento...",
        "format_error": "⚠️ Infelizmente, este formato de arquivo ainda não é suportado ou o arquivo está corrompido.",
        "text_error": "📥 Por favor, envie-me o arquivo que você deseja converter.",
        "done": "✅ Pronto! Seu arquivo foi convertido com sucesso.",
        "pick_format": "➡️ Escolha o formato de saída:",
        "file_too_large": "⚠️ Arquivo muito grande. O tamanho máximo para envio é de 20 MB.",
        "result_too_large": "⚠️ O arquivo convertido excede 50 MB e não pode ser enviado pelo Telegram.",
    },
    "ar": {
        "welcome": (
            "مرحبًا بك في محول الملفات - البوت الشامل للتحويل الفوري لجميع صيغ الملفات الشائعة!\n\n"
            "📌 أنواع الملفات المدعومة:\n"
            "📷 الصور: png, jpg, webp, heic, svg وغيرها.\n"
            "🎬 الفيديو والصور المتحركة: mp4, avi, mov, webm ← وحتى إلى mp3 أو gif!\n"
            "🎵 الصوت: mp3, flac, ogg, wav, m4a\n"
            "📄 المستندات: docx, xlsx, pptx, txt, pdf, html وغيرها.\n\n"
            "فقط أرسل الملف — وسيقوم البوت تلقائيًا باقتراح الصيغ المتاحة للتحويل.\n"
            "⚡ سريع. مريح. بدون تسجيل."
        ),
        "converting": "🔄 جارٍ تحويل الملف، يرجى الانتظار قليلاً...",
        "format_error": "⚠️ عذرًا، هذا تنسيق الملف غير مدعوم حاليًا أو أن الملف تالف.",
        "text_error": "📥 يرجى إرسال الملف الذي تريد تحويله.",
        "done": "✅ تم! تم تحويل ملفك بنجاح.",
        "pick_format": "➡️ اختر تنسيق الإخراج:",
        "file_too_large": "⚠️ الملف كبير جدًا. الحد الأقصى لحجم الرفع هو 20 ميغابايت.",
        "result_too_large": "⚠️ الملف المحوّل يتجاوز 50 ميغابايت ولا يمكن إرساله عبر تيليغرام.",
    },
    "ja": {
        "welcome": (
            "ファイルコンバーターへようこそ！あらゆる人気ファイル形式を瞬時に変換する万能ボットです。\n\n"
            "📌 対応ファイル形式：\n"
            "📷 画像：png, jpg, webp, heic, svg など\n"
            "🎬 動画・GIF：mp4, avi, mov, webm → mp3やgifへの変換も可能！\n"
            "🎵 音声：mp3, flac, ogg, wav, m4a\n"
            "📄 ドキュメント：docx, xlsx, pptx, txt, pdf, html など\n\n"
            "ファイルを送信するだけで、ボットが利用可能な変換形式を自動的に提案します。\n"
            "⚡ スピーディー。便利。登録不要。"
        ),
        "converting": "🔄 ファイルを変換しています。少々お待ちください...",
        "format_error": "⚠️ 残念ながら、このファイル形式はまだサポートされていないか、ファイルが破損しています。",
        "text_error": "📥 変換したいファイルを送信してください。",
        "done": "✅ 完了！ファイルの変換が成功しました。",
        "pick_format": "➡️ 出力形式を選択してください:",
        "file_too_large": "⚠️ ファイルが大きすぎます。アップロードの上限サイズは 20 MB です。",
        "result_too_large": "⚠️ 変換後のファイルが 50 MB を超えているため、Telegram では送信できません。",
    },
    "zh": {
        "welcome": (
            "欢迎使用文件转换器 — 一款支持所有常用文件格式即时转换的万能机器人！\n\n"
            "📌 支持的文件类型：\n"
            "📷 图片：png, jpg, webp, heic, svg 等\n"
            "🎬 视频与 GIF：mp4, avi, mov, webm → 甚至可以转换为 mp3 或 gif！\n"
            "🎵 音频：mp3, flac, ogg, wav, m4a\n"
            "📄 文档：docx, xlsx, pptx, txt, pdf, html 等\n\n"
            "只需发送文件 — 机器人会自动为您推荐可转换的格式。\n"
            "⚡ 高效、便捷、无需注册。"
        ),
        "converting": "🔄 正在转换文件，请稍候...",
        "format_error": "⚠️ 抱歉，目前暂不支持此文件格式，或者文件已损坏。",
        "text_error": "📥 请发送您想要转换的文件。",
        "done": "✅ 完成！您的文件已成功转换。",
        "pick_format": "➡️ 请选择输出格式：",
        "file_too_large": "⚠️ 文件太大，上传文件的最大限制为 20 MB。",
        "result_too_large": "⚠️ 转换后的文件超过 50 MB，无法通过 Telegram 发送。",
    },
}


def get(lang: str, key: str) -> str:
    return MESSAGES.get(lang, MESSAGES["en"])[key]
